...
class AStar {
    ...
    static LinkedList<Point> searchSolution(Maze maze) {
        // TODO: 17-10-21
        ...
        // 将 CLOSED 表初始化为空；
        LinkedList<Point> closedLinkedList = new LinkedList<>();
            ...
            // Else
                ...
                for (int i = -1; i <= 1; i++) {
                    for (int j = -1; j <= 1; j++) {
                        ...
                        if (neighborI < 0 || neighborI > maze.getPoints().length - 1) {
                            continue;
                        }
                        if (neighborJ < 0 || neighborJ > maze.getPoints()[n.getX()].length - 1) {
                            continue;
                        }
                        ...
                        Point candidate = maze.getPoints()[neighborI][neighborJ];
                        ...
                        //     Else if mi 已经在 OPEN 表中
                            ...
                            Point temp = new Point(candidate.getX(), candidate.getY());
                            temp.setParent(n);
     ...
     */
    private static boolean isAncestor(Point n, Point candidate) {
        while (n != null) {
    ...
    private static void collectionSolution(Point n, LinkedList<Point> solution) {
        if (n.getParent() != null) {
    ...
    private static int figureG(Point p, Maze maze) {
